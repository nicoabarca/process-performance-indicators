"""
Production Event Log - Indicator Analysis Example
...
DATASET_PATH = "production.csv"  # <-- CHANGE THIS to your CSV file
...
def build_indicator_arguments_simple() -> IndicatorArguments:
    ...
    return IndicatorArguments(
        ...
        case_id="CASE_001",  # Replace with an actual case ID from your log
        activity_name="Assembly",  # Replace with an activity from your log
        instance_id="CASE_001-Assembly-1",  # Replace with an instance ID (usually auto-generated)
        human_resource_name="Worker_5",  # Replace with a resource name (if you have human_resource column)
        # Multiple entities - for subset/comparison indicators
        case_ids={"CASE_001", "CASE_002", "CASE_003"},
        automated_activities={"Data_Entry", "Email_Notification"},  # Activities performed by machines
        desired_activities={"Quality_Check", "Final_Inspection"},  # Activities you want to see
        unwanted_activities={"Rework", "Error_Correction"},  # Activities to avoid
        direct_cost_activities={"Assembly", "Testing", "Packaging"},  # Activities with direct costs
        activities_subset={"Assembly", "Quality_Check"},  # Subset for specific analysis
        # Cross-activity parameters - for sequence/relationship indicators
        activity_a="Order_Received",  # First activity in a sequence
        activity_b="Order_Completed",  # Second activity in a sequence
        # Time window parameters - for time-bounded indicators
        start_time=None,  # Optional: pd.Timestamp("2023-01-01") to filter events after this
        end_time=None,  # Optional: pd.Timestamp("2023-12-31") to filter events before this
        # Organizational parameters
        role_name=None,  # Optional: "Manager" or "Operator" (if you have role column)
        # Threshold/expectation parameters - for compliance indicators
        deadline=pd.Timedelta(days=7),  # Expected maximum time (use pd.Timedelta for time-based)
        expectation=pd.Timedelta(days=3.5),  # Expected typical time
        value=1,  # Threshold value for rework counting (e.g., activities repeated > 1 time)
        # Aggregation mode
